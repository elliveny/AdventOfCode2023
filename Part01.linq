...
let processLine (line : string, part2 : bool) = 
    let paddedLine = line + "         "
    [|
    for strIndex in 0 .. line.Length - 1 do
        if (Regex.IsMatch(line.Substring(strIndex,1), "\d")) then
            yield line.Substring(strIndex,1) |> int;
        else
            if part2 then
                if System.String.Equals(paddedLine.Substring(strIndex,3),"one") then yield 1;
                if System.String.Equals(paddedLine.Substring(strIndex,3),"two") then yield 2;
                if System.String.Equals(paddedLine.Substring(strIndex,5),"three") then yield 3;
                if System.String.Equals(paddedLine.Substring(strIndex,4),"four") then yield 4;
                if System.String.Equals(paddedLine.Substring(strIndex,4),"five") then yield 5;
                if System.String.Equals(paddedLine.Substring(strIndex,3),"six") then yield 6;
                if System.String.Equals(paddedLine.Substring(strIndex,5),"seven") then yield 7;
                if System.String.Equals(paddedLine.Substring(strIndex,5),"eight") then yield 8;
                if System.String.Equals(paddedLine.Substring(strIndex,4),"nine") then yield 9;
    |]

let firstLast (input : int[]) = 
    $"{input.First()}{input.Last()}" |> int

let result (input : string[], part2 : bool) = 
    let total = Array.sum ([| 
        for lineIdx in 0 .. input.Length - 1 do
            yield firstLast (processLine (input[lineIdx], part2))
    |])
    total

let part1TestResult = result (testPart1Input, false)
let part1Result = result (input, false)
let part2TestResult = result (testPart2Input, true)
let part2Result = result (input, true)


printfn $"Part 1 Test: {part1TestResult}"
printfn $"Part 1: {part1Result}"
printfn ""
printfn $"Part 2 Test: {part2TestResult}"
printfn $"Part 2: {part2Result}"
